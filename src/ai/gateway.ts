...
export class AlfredAIGateway {
  ...
  async openAI(model: string, messages: any[], options?: { temperature?: number; max_tokens?: number }): Promise<any> {
    ...
    const response = await fetch(`${this.gatewayBase}/openai/v1/chat/completions`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model, messages, ...options }) });
  ...
  async mimo(model: string, messages: any[], options?: { temperature?: number; max_tokens?: number }): Promise<any> {
    const apiKey = await this.env.MIMO_API_KEY.get();
    const response = await fetch(`${this.gatewayBase}/custom-mimo/v1/chat/completions`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model, messages, ...options }) });
  ...
  async xai(model: string, messages: any[], options?: { temperature?: number; max_tokens?: number }): Promise<any> {
    const apiKey = await this.env.XAI_API_KEY.get();
    const response = await fetch(`${this.gatewayBase}/custom-xai/v1/chat/completions`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model, messages, ...options }) });
  ...
  async chat(messages: any[], userId?: string, options?: { temperature?: number; max_tokens?: number }): Promise<any> {
    ...
    switch (provider) {
      ...
      case "mimo": return this.mimo("mimo-v2.5-pro", messages, options);

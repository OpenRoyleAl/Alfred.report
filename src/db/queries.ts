...
export async function createMission(env: Env, data: { user_id: string; title: string; description?: string; priority?: number; oral_directives?: string; budget_usd?: number }): Promise<Mission> {
  const id = crypto.randomUUID();
  const now = new Date().toISOString();
  await env.DB.prepare(`INSERT INTO missions (id, user_id, title, description, status, priority, budget_usd, oral_directives, created_at, updated_at) VALUES (?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?)`).bind(id, data.user_id, data.title, data.description || null, data.priority || 5, data.budget_usd ?? null, data.oral_directives || null, now, now).run();

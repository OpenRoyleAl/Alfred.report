...
export class MissionStateDO extends DurableObject<Env> {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    const suppliedMissionId = url.searchParams.get("missionId");
    if (suppliedMissionId) {
      this.currentMissionId = suppliedMissionId;
      await this.ctx.storage.put("missionId", suppliedMissionId);
    } else {
      this.currentMissionId = (await this.ctx.storage.get("missionId")) as string || "unknown";
    }
  ...
  async startMission(): Promise<Response> {
    ...
    const mission = await this.env.DB.prepare(`SELECT budget_usd FROM missions WHERE id = ?`).bind(this.missionId).first<{ budget_usd: number | null }>();
    const storedBudget = (await this.ctx.storage.get("budgetUsd")) as number | undefined;
    const budget = storedBudget ?? mission?.budget_usd ?? undefined;
    if (budget !== undefined) await this.ctx.storage.put("budgetUsd", budget);
  ...
  private get missionId(): string { return this.currentMissionId; }
}
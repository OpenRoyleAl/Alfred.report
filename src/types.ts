...
export interface AgentMemoryNamespace {
  ingest(input: { messages: any[]; profile?: string }): Promise<void>;
  remember(input: { content: string; profile?: string; metadata?: Record<string, string> }): Promise<void>;
  recall(input: { query: string; profile?: string; topK?: number }): Promise<any[]>;
  getSummary(input: { profile?: string }): Promise<string | null>;
}

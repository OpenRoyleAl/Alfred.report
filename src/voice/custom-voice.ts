...
export async function handleVoiceUpload(request: Request, env: Env, userId: string): Promise<Response> {
  ...
  await env.DB.prepare(`INSERT INTO user_preferences (user_id, voice_sample_key, updated_at) VALUES (?, ?, datetime('now')) ON CONFLICT(user_id) DO UPDATE SET voice_sample_key = ?, updated_at = datetime('now')`).bind(userId, key, key).run();

...
import type { Env, TTSProvider, TTSOptions } from "../types";
...
class GatewayTTS implements TTSProvider {
  ...
  async synthesize(text: string, options?: TTSOptions): Promise<ArrayBuffer> {
    ...
    const response = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${this.apiKey}` }, body: JSON.stringify({ model: options?.voice || "tts-1", input: text, voice: options?.voice || "alloy" }) });
...
class OpenAITTS implements TTSProvider {
  ...
  async synthesize(text: string, options?: TTSOptions): Promise<ArrayBuffer> {
    ...
    const response = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${this.apiKey}` }, body: JSON.stringify({ model: "tts-1", input: text, voice: options?.voice || "alloy" }) });
